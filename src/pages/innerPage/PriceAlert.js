...
import PriceAlertData from "../../content/PriceAlert.json";
import { Link } from "react-router-dom";
import useScreenWidth from "../../component/function/screenwidth";
import PriceAlertDemo from "../../component/function/priceAlertDemo";

const PriceAlert = () => {
  const width = useScreenWidth();
  const { language } = useLanguage();
  const buttonText = language === "English" ? "Back to List" : "返回選單";
  const projectIntro = PriceAlertData.find((item) => item.id === 1);
  const projectContent = PriceAlertData.find((item) => item.id === 2);
